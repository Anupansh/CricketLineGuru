//
//  SeasonSeriesMatchesAdCell.swift
//  cricrate
//
...
class SeasonSeriesMatchesAdCell: UITableViewCell {
        ...
        if let status = data.status, status == "completed"{
            ...
            if let start_date = data.st_date{
                ...
                if matchDate != nil{
                    ...
                    //if AppHelper.currentTimeInMiliseconds(currentDate:matchDate!,format:"dd MMM hh:mm a") <  AppHelper.currentTimeInMiliseconds(currentDate:Date(),format:"dd MMM hh:mm a"){
                    if matchDate!.currentTimeMillis() <  Date().currentTimeMillis(){
                        if let status = data.status, status == "completed"{
                            if let msgs = data.msgs{
                                if let info = msgs.info{
                                    self.LblResult.text = info
