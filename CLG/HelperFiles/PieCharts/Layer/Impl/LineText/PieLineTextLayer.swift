...
public struct PieLineTextLayerSettings {
    ...
    public var segment2Length: CGFloat = 30
    public var lineColor: UIColor = UIColor.black
    public var lineWidth: CGFloat = 2
    public var chartOffset: CGFloat = -13
...
open class PieLineTextLayer: PieChartLayer {
    ...
    public func addItems(slice: PieSlice) {
        ...
        let p1 = slice.view.calculatePosition(angle: slice.view.midAngle, p: CGPoint(x: slice.view.center.x, y: slice.view.center.y), offset: slice.view.outerRadius + settings.chartOffset)
        let p2 = slice.view.calculatePosition(angle: slice.view.midAngle, p: CGPoint(x: slice.view.center.x, y: slice.view.center.y ), offset: slice.view.outerRadius + settings.segment1Length)
        ...
        let circleView = UIView(frame: CGRect(origin: p1, size: CGSize(width: 5, height: 5)))
        circleView.center = p1
        circleView.layer.cornerRadius = circleView.frame.height/2
        circleView.backgroundColor = UIColor.white
        chart?.container.addSublayer(lineLayer)
        animator.animate(lineLayer)
        
        chart?.container.addSublayer(circleView.layer)
        animator.animate(circleView.layer)
    ...
    public func createLabel(slice: PieSlice, isRightSide: Bool, referencePoint: CGPoint) -> UILabel {
        ...
        let label: UILabel = settings.label.labelGenerator?(slice) ?? {
            ...
            label.textColor = settings.label.textColor(slice)
